...
def execute_source_run_locally(db: Session, source: Source, organization_id: str | None = None) -> SourceRun:
    ...
    try:
        ...
        try:
            ...
            for opportunity_data in opportunities:
                try:
                    opportunity = loop.run_until_complete(
                        create_opportunity(db, opportunity_data, organization_id=organization_id)
                    )
            ...
            # Close any per-loop httpx.AsyncClient before closing the loop.
            from app.core.http_client import close_per_loop_client

            try:
                loop.run_until_complete(close_per_loop_client())
            except Exception:
                pass
            loop.close()
        db.flush()
