    ...
    *,
    ...
    if close_date_to:
        try:
            ...
            stmt = stmt.where(
                Opportunity.close_date <= to_date.replace(hour=23, minute=59, second=59)
            )
        except ValueError:
            pass
    if min_amount is not None:
        stmt = stmt.where(
            Opportunity.funding_amount_value.isnot(None),
            Opportunity.funding_amount_value >= min_amount,
        )
    if max_amount is not None:
        stmt = stmt.where(
            Opportunity.funding_amount_value.isnot(None),
            Opportunity.funding_amount_value <= max_amount,
        )
    if priority:
...
def _text_search_opportunities(
    ...
    *,
    ...
    tokens = [
        t.translate(_ACCENT_MAP)
        for t in re.findall(r"[a-zA-ZáéíóúñüÁÉÍÓÚÑÜ]+", query)
        if len(t) >= 2
    ]
    if not tokens:
        return []
...
def _lexical_search_score(query_terms: set[str], opportunity: Opportunity) -> float:
    ...
    category_boost = (
        0.1
        if any(term in " ".join(opportunity.categories or []).lower() for term in query_terms)
        else 0.0
    )
