...
def test_concurrency_block_present():
    ...
    assert "concurrency" in DATA, "Missing concurrency block — overlapping deploys could race"
...
def test_render_api_call_uses_official_endpoint():
    ...
    assert "RENDER_API_BASE" in api_steps, "render-api job must reference RENDER_API_BASE"
...
def test_deploy_render_yml_is_removed():
    ...
    old_workflow = (
        Path(__file__).parent.parent.parent.parent / ".github" / "workflows" / "deploy-render.yml"
    )
