...
from worker.connectors.eu_funding_tenders import (
    EU_TOPIC_URL,
    _clean,
    _first_text,
    _is_openish,
...
EIC_SEARCH_URL = "https://api.tech.ec.europa.eu/search-api/prod/rest/search"
...
class EicAcceleratorConnector:
    ...
    def __init__(self, base_url: str | None = None) -> None:
        self.base_url = base_url or EIC_SEARCH_URL

    async def fetch(self) -> RawSourceResult:
        results: list[dict[str, object]] = []
        seen: set[str] = set()
        final_url = self.base_url
        for term in EIC_TERMS:
            search_url = (
                f"{EIC_SEARCH_URL}?apiKey=SEDIA&text={quote_plus(term)}&pageSize=50&pageNumber=1"
            )
            ...
            for item in payload.get("results") or []:
                ...
                if not identifier or identifier in seen:
                    continue
                if "EIC" not in identifier.upper() and "EIC" not in _clean(_first_text(metadata.get("callTitle"))).upper():
                    continue
                seen.add(identifier)
                results.append(item)
        ...
        return RawSourceResult(
            ...
            metadata={"terms": EIC_TERMS, "result_count": len(results)},
        )
        ...
        for item in items:
            ...
            candidates.append(
                OpportunityCandidate(
                    title=title[:180],
                    entity="European Innovation Council",
                    country="European Union",
                    official_url=EU_TOPIC_URL.format(identifier=quote_plus(identifier)),
                    summary=_clean(item.get("summary") or title)[:700],
                    categories=["grants", "innovation", "startup", "eic accelerator"],
                    topics=keywords[:5] or ["EIC", "accelerator"],
